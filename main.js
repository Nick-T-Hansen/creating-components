...
const h1 = (title, style, color) => {
    const h1Element = document.createElement('h1');
    h1Element.textContent = title;
    h1Element.classList.add(style);
    h1Element.classList.add(color);
    return h1Element;
}
...
const student = (name, className, info) => {
    ...
    divElement.appendChild(h1(name, "xx-large", "passing"));
...
const studentFail = (name, className, info) => {
    const divElement = document.createElement('div');
    divElement.appendChild(h1(name, "xx-large", "failing"));
    divElement.appendChild(section(className, "section-padded"));
    divElement.appendChild(aside(info, "pushRight"));
    return divElement;
}
for ( i = 0; i < students.length; i++) {
    if (students[i].score >= 60) {
        containerRef.appendChild(student(students[i].name, students[i].class, students[i].info));
            ...
            containerRef.appendChild(studentFail(students[i].name, students[i].class, students[i].info));
        }
    }
